...
class DeepSeekProvider(BaseLLMProvider):
    """DeepSeek provider (R1 reasoner, V3 chat) via OpenAI-compatible endpoint."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if AsyncOpenAI is None:
            raise ImportError(
                "The 'openai' package is required to use DeepSeekProvider. "
    ...
    async def generate(
        ...
        """Generate a response via the DeepSeek OpenAI-compatible endpoint."""
        ...
        try:
            ...
            logger.info(
                "DeepSeek [%s] responded in %.0fms (%d tokens)",
            ...
            logger.error("DeepSeek [%s] error: %s", self.model, exc)
            return LLMResponse(
                content="",
                model=self.model,
                provider="deepseek",
